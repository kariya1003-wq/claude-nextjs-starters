...
이 프로젝트는 1인 프리랜서 및 소규모 사업자가 노션에 입력한 견적서를 클라이언트에게 깔끔한 웹 페이지로 공유하고, PDF로 다운로드할 수 있게 하는 Next.js 15.5.7 기반 웹 애플리케이션입니다.
...
> 다크모드 지원, 관리자 대시보드 레이아웃 개선, 목록에서 링크 복사 등 사용성을 한 단계 끌어올리는 단계

- **Task 019: 다크모드 테마 시스템 구현** ✅ - 완료
  - ✅ `next-themes` ThemeProvider를 루트 레이아웃에 적용 (attribute="class", defaultTheme="system")
  - ✅ 헤더에 라이트/다크/시스템 테마 전환 토글 버튼 구현 (Sun/Moon 아이콘, shadcn/ui DropdownMenu 활용)
  - ✅ `globals.css`에 `.dark` 클래스 기반 다크모드 CSS 변수 완성 (oklch 컬러 팔레트)
  - ✅ shadcn/ui 컴포넌트 및 커스텀 컴포넌트의 다크모드 스타일 검증
  - ✅ 뷰어 페이지(`/viewer/*`)는 항상 라이트 모드 강제 (PDF 출력 일관성 보장)
  - ✅ 테마 전환 시 깜빡임(FOUC) 방지 처리 (suppressHydrationWarning)

- **Task 020: 관리자 대시보드 레이아웃 구현** ✅ - 완료
  - ✅ 관리자 전용 레이아웃 컴포넌트 생성 (`components/layout/admin-layout.tsx`)
  - ✅ 사이드바 내비게이션 구현 (견적서 목록, 대시보드 메뉴, 반응형 접기/펼치기)
  - ✅ 견적서 상태별 요약 카드 영역 구현 (초안/발송/확정/만료 건수 표시)
  - ✅ 기존 목록 페이지(`/`)를 관리자 대시보드 레이아웃으로 전환
  - ✅ 모바일에서는 사이드바를 시트(Sheet) 또는 드로어(Drawer) 형태로 전환

- **Task 021: 견적서 목록 필터링 및 검색 기능 구현** ✅ - 완료
  - ✅ 상태별 필터 탭 또는 드롭다운 구현 (전체/초안/발송/확정/만료)
  - ✅ 견적번호 또는 클라이언트명 기반 텍스트 검색 입력 구현
  - ✅ URL 쿼리 파라미터 기반 필터/검색 상태 관리 (useSearchParams)
  - ✅ Notion API 쿼리에 필터 조건 반영 (서버사이드 필터링)
  - ✅ 필터/검색 결과가 없을 때 빈 상태 UI 표시

- **Task 022: 견적서 목록에서 링크 복사 기능 구현** ✅ - 완료
  - ✅ 견적서 목록 각 행/카드에 "링크 복사" 액션 버튼 추가 (Link/Copy 아이콘)
  - ✅ 클라이언트 뷰어 URL(`/viewer/[quoteNumber]`) 클립보드 복사 기능 구현
  - ✅ 복사 성공 시 Sonner 토스트 알림 표시
  - ✅ 복사 완료 상태 피드백 (아이콘 전환 애니메이션, Task 013 패턴 재활용)
  - ✅ 비공개(isPublic=false) 견적서는 복사 불가 처리 및 안내 메시지 표시

- **Task 023: Phase 5 통합 테스트 및 마무리** ✅ - 완료
  - ✅ Playwright MCP를 사용한 다크모드 전환 E2E 테스트 (라이트/다크/시스템)
  - ✅ 관리자 대시보드 레이아웃 반응형 테스트 (데스크톱/태블릿/모바일)
  - ✅ 필터링 + 검색 + 링크 복사 통합 플로우 테스트
  - ✅ 다크모드 상태에서 전체 페이지 스타일 일관성 검증
  - ✅ 뷰어 페이지 라이트 모드 강제 동작 확인
...
**📅 최종 업데이트**: 2026-04-07
**📊 진행 상황**: 전체 완료 (23/23 Tasks 완료, 100%)
